...
def crop(original_img, crop_shape, crop_position, method="center",
    ...
    This function must stay in numpy since original_img should not be loaded into Pytorch during the network time.
...
def get_max_window(input_image, window_shape, pooling_logic="avg"):
    ...
    input_image and returns the UPPER_LEFT corner index with max sum.
    :param input_image: N*C*H*W
    :param window_shape: h*w
    :param pooling_logic: method of pooling, one of: avg, std, avg_entropy.
    :return: N*C*2 tensor
    """
...
def generate_mask_uplft(input_image, window_shape, upper_left_points, gpu_number):
    """
    Function that generates a mask that crops given upper_left corners to 0.
    """
    N, C, H, W = input_image.size()
