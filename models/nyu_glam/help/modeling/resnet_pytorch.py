...
class BasicBlock2by2(BasicBlock):
    """
    Class that replace the first 3*3 conv with a 2*2 conv if stride=2.
    """

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BasicBlock2by2, self).__init__(inplanes, planes, stride, downsample)
        if self.stride == 2:
            self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=2, stride=stride, bias=False)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, inplanes=64, num_classes=1000):
